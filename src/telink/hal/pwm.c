...
static uint8_t pin_to_pwm_id(hal_gpio_pin_t pin) {
    // TLSR8258 PWM channel mapping (from SDK documentation):
    // PWM0: PB4, PC2, PA0
    // PWM1: PB5, PC3, PA7
    // PWM2: PB7, PC4, PD2
    // PWM3: PC0, PD3
    ...
    if (port == 1) { // Port B
        if (pin_num == 4) return 0; // PB4 -> PWM0
        if (pin_num == 5) return 1; // PB5 -> PWM1
        if (pin_num == 7) return 2; // PB7 -> PWM2
    } else if (port == 2) { // Port C
        if (pin_num == 2) return 0; // PC2 -> PWM0
        if (pin_num == 3) return 1; // PC3 -> PWM1
        if (pin_num == 4) return 2; // PC4 -> PWM2
        if (pin_num == 0) return 3; // PC0 -> PWM3
        if (pin_num == 1) return 4; // PC1 -> PWM4
    } else if (port == 3) { // Port D
        if (pin_num == 2) return 2; // PD2 -> PWM2
        if (pin_num == 3) return 3; // PD3 -> PWM3
        if (pin_num == 4) return 4; // PD4 -> PWM4
        if (pin_num == 5) return 5; // PD5 -> PWM5
    } else if (port == 0) { // Port A
        if (pin_num == 0) return 0; // PA0 -> PWM0
        if (pin_num == 7) return 1; // PA7 -> PWM1
    }
